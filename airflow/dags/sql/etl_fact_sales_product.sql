...
-- Runs every 5 minutes via Airflow DAG (etl_fact_sales_product)
-- Joins: silver.orders + silver.order_items + silver.products
-- Filter: Only delivered orders (order_status_id = 4)
-- =============================================================================
--
-- Discount allocation formula per product line item:
--   Discount_Item = (Price_Item * Qty / Order_Amount) * Order_Discount
--
-- This ensures order-level discounts are proportionally distributed across
-- line items based on their contribution to the total order value.
-- =============================================================================
...
WHERE o.order_status_id = 4  -- delivered only
  AND toDate(o.created_at) >= today() - 1  -- process recent data
GROUP BY
