...
SELECT
    ...
    category_id,
    brand_id,
    toDecimal64OrDefault(product_price, 2, 0) AS product_price,
    toDecimal64OrDefault(unit_cost, 2, 0)     AS unit_cost,
...
SELECT
    ...
    adscampaign_id,
    type,
    toDecimal64OrDefault(value, 2, 0) AS value,
    code,
    if(started_at IS NOT NULL, fromUnixTimestamp64Milli(started_at), NULL) AS started_at,
    if(expired_at IS NOT NULL, fromUnixTimestamp64Milli(expired_at), NULL) AS expired_at,
    __source_ts_ms AS _version
...
SELECT
    ...
    staff_id,
    address_id,
    toDecimal64OrDefault(order_amount, 2, 0)    AS order_amount,
    toDecimal64OrDefault(discount_amount, 2, 0) AS discount_amount,
    toDecimal64OrDefault(total_amount, 2, 0)    AS total_amount,
    discount_id,
    payment_method_id,
    payment_status_id,
    order_status_id,
...
SELECT
    ...
    quantity,
    toDecimal64OrDefault(product_price, 2, 0)   AS product_price,
    toDecimal64OrDefault(product_tax, 2, 0)      AS product_tax,
    toDecimal64OrDefault(subtotal_amount, 2, 0)  AS subtotal_amount,
    COALESCE(fromUnixTimestamp64Milli(created_at), now()) AS created_at,
    __source_ts_ms AS _version
